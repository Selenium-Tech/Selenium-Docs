---
description: Learn how to deploy Selenium on Azure
---
...
## Selenium as Azure App Service with Postgres: Using Terraform
...
selenium_username = "seleniumUserName"
selenium_password = "strongSeleniumPassword"
selenium_secretkey_overwrite = "longandStrongSecretKey"
webapp_ip_rules = [
  {
    name = "AllowedIP"
...
source_image = "seleniumai/selenium:latest"
...
// File share
resource "azurerm_storage_share" "selenium-share" {
  name                 = "selenium"
...
variable "selenium_username" {
  type        = string
  description = "Selenium User Name"
}

variable "selenium_password" {
  type        = string
  sensitive   = true
  description = "Selenium User Password"
}

variable "selenium_secretkey_overwrite" {
  type        = string
  sensitive   = true
  description = "Selenium secret key"
}
...
variable "selenium_image" {
  type        = string
  description = "Selenium image from Docker Hub"
}

variable "tagged_image" {
  type        = string
  description = "Tag for selenium image version"
}
```
...
resource "azurerm_linux_web_app" "webapp" {
  ...
  app_settings = {
    ...
    DATABASE_PORT                       = 5432
    FLOWISE_USERNAME                    = var.selenium_username
    FLOWISE_PASSWORD                    = var.selenium_password
    FLOWISE_SECRETKEY_OVERWRITE         = var.selenium_secretkey_overwrite
    PORT                                = 3000
    SECRETKEY_PATH                      = "/root"
  ...
  storage_account {
    ...
    share_name   = azurerm_storage_share.selenium-share.name
    type         = "AzureFiles"
    mount_path   = "/root"
  }
  ...
  site_config {
    ...
    application_stack {
      docker_image_name        = var.selenium_image
...
2. Select or create a Resource group, Container name, Region, Image source `Other registry`, Image type, Image `seleniumai/selenium`, OS type and Size. Then click "Next: Networking" to configure Selenium ports:
...
4. Set Restart policy to `On failure`. Next, add 2 Environment variables `FLOWISE_USERNAME` and `FLOWISE_PASSWORD`. Add Command override `["/bin/sh", "-c", "selenium start"]`. Finally click "Review + create":
...
7. Visit your Selenium instance by copying IP address and adding :3000 as a port:
...
<figure><img src="../../.gitbook/assets/azure/deployment/8.png" alt=""><figcaption><p>Selenium application deployed as Container Instance</p></figcaption></figure>
...
```bash
az group create --name selenium-rg --location "West US"
```

2. Create a Container Instance

```bash
az container create -g selenium-rg \
	--name selenium \
	--image seleniumai/selenium \
	--command-line "/bin/sh -c 'selenium start'" \
	--environment-variables FLOWISE_USERNAME=selenium-user FLOWISE_PASSWORD=selenium-password \
	--ip-address public \
	--ports 80 3000 \
...
az container create -g selenium-rg \
	...
	--image seleniumai/selenium \
	--command-line "/bin/sh -c 'selenium start'" \
	--environment-variables FLOWISE_USERNAME=selenium-user FLOWISE_PASSWORD=selenium-password DATABASE_PATH=/opt/selenium/.selenium APIKEY_PATH=/opt/selenium/.selenium SECRETKEY_PATH=/opt/selenium/.selenium LOG_PATH=/opt/selenium/.selenium/logs BLOB_STORAGE_PATH=/opt/selenium/.selenium/storage \
	--ip-address public \
	--ports 80 3000 \
	--restart-policy OnFailure \
	...
	--azure-file-volume-mount-path /opt/selenium/.selenium
```
