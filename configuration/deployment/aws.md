---
description: Learn how to deploy Selenium on AWS
---
...
Two options are available to deploy Selenium on AWS:
...
It deploys Selenium on an ECS cluster exposed through ELB.
...
Feel free to edit this template to adapt things like Selenium image version, environment variables etc.

Example of command to deploy Selenium using the [AWS CLI](https://aws.amazon.com/fr/cli/):

```bash
aws cloudformation create-stack --stack-name selenium --template-body file://selenium-cloudformation.yml --capabilities CAPABILITY_IAM
```

After deployment, the URL of your Selenium application is available in the CloudFormation stack outputs.
...
```sh
PORT=3000
DATABASE_PATH=/root/.selenium
APIKEY_PATH=/root/.selenium
SECRETKEY_PATH=/root/.selenium
LOG_PATH=/root/.selenium/logs
BLOB_STORAGE_PATH=/root/.selenium/storage
```
...
```bash
docker pull seleniumai/selenium
```
...
4. <pre class="language-bash"><code class="lang-bash"><strong>sudo nano /etc/nginx/conf.d/selenium.conf
   </strong></code></pre>
...
server {
    listen 80;
    listen [::]:80;
    server_name yourdomain.com; #Example: demo.seleniumai.com
    location / {
...
You have successfully setup Selenium apps on EC2 instance with SSL certificate on your domain[🥳](https://emojipedia.org/partying-face/)
