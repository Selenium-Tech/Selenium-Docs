---
description: Learn how to deploy Selenium on GCP
---
...
```
docker build -t gcr.io/[ProjectId]/selenium:dev .
```

4. Push the Docker image to GCP container registry.

```
docker push gcr.io/[ProjectId]/selenium:dev
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: selenium
        image: gcr.io/[ProjectID]/selenium:dev
...
6. Go to `Services & Ingress`, you can click the `Endpoint` where the Selenium is hosted.

<figure><img src="../../.gitbook/assets/gcp/5.png" alt=""><figcaption></figcaption></figure>

## Congratulations!

You have successfully hosted the Selenium apps on GCP [🥳](https://emojipedia.org/partying-face/)
...
metadata:
  annotations:
    cloud.google.com/backend-config: '{"default": "selenium-backendconfig"}'
  name: selenium-service
  namespace: your-namespace
...
```
