---
description: Learn how to use the Selenium Prediction, Vector Upsert and Message API
---
...
Some document loaders in Selenium allow user to upload files:
...
Message API is restricted to only Selenium admin user. Basic authentication must be provided in the headers if Selenium instance has been configured with `FLOWISE_USERNAME` and `FLOWISE_PASSWORD`. Refer [app-level.md](../configuration/authorization/app-level.md "mention") for more details.
