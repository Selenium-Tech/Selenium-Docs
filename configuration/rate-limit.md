---
description: Learn how to managing API requests in Selenium
---
...
Using the example above, that means only 20 messages are allowed to be received in 60 seconds. The rate limitation is tracked by IP-address. If you have deployed Selenium on cloud service, you'll have to set `NUMBER_OF_PROXIES` env variable.

## Cloud-Hosted Rate Limit Setup Guide

1. **Cloud Host Selenium:** Start by hosting Selenium in the cloud.
...
3. **Restart Cloud-Hosted Selenium Service:** This enables Selenium to apply changes of environment variables.
...
6. **Incorrect IP Address:** If the returned IP address does not match your current IP address, increase `NUMBER_OF_PROXIES` by 1 and restart Cloud-Hosted Selenium. Repeat this process until the IP address matches your own.
